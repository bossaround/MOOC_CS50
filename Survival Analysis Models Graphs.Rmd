...
library(dplyr)
 ...
 d.discrete.complete.age <- sample_n(dat8,10000) 
...
r1 <- ggplot()+
     geom_smooth(data=pointestimate.age.level[which(pointestimate.age.level$time<8 & pointestimate.age.level$age.level=="18 below"),],aes(time,pmean+rt*pse, color=as.factor(age.level)),  method = "glm",formula = formula,se=FALSE)+
      geom_smooth(data=pointestimate.age.level[which(pointestimate.age.level$time<8 & pointestimate.age.level$age.level=="18 below"),],aes(time,pmean-rt*pse, color=as.factor(age.level)),  method = "glm",formula = formula,se=FALSE)
...
r2 <- ggplot()+
     geom_smooth(data=pointestimate.age.level[which(pointestimate.age.level$time<8 & pointestimate.age.level$age.level=="18-30"),],aes(time,pmean+rt*pse, color=as.factor(age.level)), method = "glm",formula = formula,se=FALSE)+
      geom_smooth(data=pointestimate.age.level[which(pointestimate.age.level$time<8 & pointestimate.age.level$age.level=="18-30"),],aes(time,pmean-rt*pse, color=as.factor(age.level)), method = "glm",formula = formula,se=FALSE)
...
r3 <- ggplot()+
     geom_smooth(data=pointestimate.age.level[which(pointestimate.age.level$time<8 & pointestimate.age.level$age.level=="30-45"),],aes(time,pmean+rt*pse, color=as.factor(age.level)), method = "glm",formula = formula,se=FALSE)+
      geom_smooth(data=pointestimate.age.level[which(pointestimate.age.level$time<8 & pointestimate.age.level$age.level=="30-45"),],aes(time,pmean-rt*pse, color=as.factor(age.level)), method = "glm",formula = formula,se=FALSE)
...
r4 <- ggplot()+
     geom_smooth(data=pointestimate.age.level[which(pointestimate.age.level$time<8 & pointestimate.age.level$age.level=="45-60"),],aes(time,pmean+rt*pse, color=as.factor(age.level)), method = "glm",formula = formula,se=FALSE)+
      geom_smooth(data=pointestimate.age.level[which(pointestimate.age.level$time<8 & pointestimate.age.level$age.level=="45-60"),],aes(time,pmean-rt*pse, color=as.factor(age.level)), method = "glm",formula = formula,se=FALSE)
...
r5 <- ggplot()+
     geom_smooth(data=pointestimate.age.level[which(pointestimate.age.level$time<8 & pointestimate.age.level$age.level=="60 above"),],aes(time,pmean+rt*pse, color=as.factor(age.level)), method = "glm",formula = formula,se=FALSE)+
      geom_smooth(data=pointestimate.age.level[which(pointestimate.age.level$time<8 & pointestimate.age.level$age.level=="60 above"),],aes(time,pmean-rt*pse, color=as.factor(age.level)), method = "glm",formula = formula,se=FALSE)
...
formula <- y~x
pmean1.log <- rep(NA,9)
for (i in 1:9) {pmean1.log[i] <- mean(newdat.discrete.cont$predict_log[which(newdat.discrete.cont$milestones==i & newdat.discrete.cont$foreign==1)],na.rm = TRUE)
  
}
pmean1.log <- pmean1.log+0.02
pmean0.log <- rep(NA,9)
for (i in 1:9) {pmean0.log[i] <- mean(newdat.discrete.cont$predict_log[which(newdat.discrete.cont$milestones==i & newdat.discrete.cont$foreign==0)],na.rm = TRUE)
  
}
pmean0.log<-pmean0.log-0.02
pse1.log <- rep(NA,9)
for (i in 1:9) {pse1.log[i] <- 
sd(newdat.discrete.cont$predict_log[newdat.discrete.cont$foreign==1 ],na.rm = TRUE)/sqrt(length(newdat.discrete.cont$predict_probability[which(newdat.discrete.cont$milestones==i & newdat.discrete.cont$foreign==1 )]))
}

pse0.log <- rep(NA,9)
for (i in 1:9) {pse0.log[i] <- sd(newdat.discrete.cont$predict_log[newdat.discrete.cont$foreign==0 ],na.rm = TRUE)/sqrt(length(newdat.discrete.cont$predict_probability[which(newdat.discrete.cont$milestones==i)]))
...
foreign <- c(rep(1,9),rep(0,9))
#pointestimate <- data.frame(pmean1,pmean0,pse1,pse0)
pmean.log<-c(pmean1.log,pmean0.log)
pse.log <- c(pse1.log,pse0.log)
pointestimate.foreign.log<-data.frame(pmean.log,pse.log,time,foreign)
pointestimate.foreign.log

r0 <- ggplot()+
     geom_smooth(data=pointestimate.foreign.log[which(pointestimate.foreign.log$time<8 & pointestimate.foreign$foreign==0),],aes(time,pmean.log+1.9*pse.log, color=as.factor(foreign)), method = "glm",formula = formula,se=FALSE)+
      geom_smooth(data=pointestimate.foreign.log[which(pointestimate.foreign.log$time<8 & pointestimate.foreign$foreign==0),],aes(time,pmean.log-1.9*pse.log, color=as.factor(foreign)), method = "glm",formula = formula,se=FALSE)+
    scale_x_continuous(breaks = seq(1,9,by=1),
                     labels = labs)
...
r1 <- ggplot()+
     geom_smooth(data=pointestimate.foreign.log[which(pointestimate.foreign.log$time<8 & pointestimate.foreign$foreign==1),],aes(time,pmean.log+1.9*pse.log, color=as.factor(foreign)),  method = "glm",formula = formula,se=FALSE)+
      geom_smooth(data=pointestimate.foreign.log[which(pointestimate.foreign.log$time<8 & pointestimate.foreign$foreign==1),],aes(time,pmean.log-1.9*pse.log, color=as.factor(foreign)),  method = "glm",formula = formula,se=FALSE)+
    scale_x_continuous(breaks = seq(1,9,by=1),
                     labels = labs)
...
ggplot() +
      geom_smooth(data=pointestimate.foreign.log[which(pointestimate.foreign.log$time<8 ),],aes(time,pmean.log, color=as.factor(foreign)), method = "glm",formula=formula,se=FALSE) +
  geom_ribbon(data=dfr,aes(x = x, ymin = ymin, ymax = ymax,fill=as.factor(foreign)),
              alpha = 0.4) + 
   geom_pointrange(data=pointestimate.foreign.log,aes(time,pmean.log, ymin=pmean.log-1.9*pse.log, ymax=pmean.log+1.9*pse.log, color=as.factor(foreign)),position=position_dodge(width = 0.1),size=0.8,fatten = 1.5)+
    scale_x_continuous(breaks = seq(1,9,by=1),
                     labels = labs)+
   xlab("Milestone") + ylab("Log Odd of Dropout") + scale_fill_discrete(name="Location",labels = c("US","Outside US")) + scale_color_discrete(name="Location",labels = c("US","Outside US")) + 
   scale_y_continuous(limits=c(-2, 2.5))+ theme_light() + theme(legend.position = "bottom")
ggsave(filename = "dropout by foreign log odd.pdf",device = "pdf")
```
