...
labs <- c("p1", "p2", "p3", "p4","p5","p6","p7","p8 (optional)","Final")
 ...
 ggplot()+
  geom_smooth (data= newdat.discrete[newdat.discrete$milestones>0,], method="glm", formula = y~x,  alpha=0.65, size = 1,level=0.95, aes(milestones, predict_probability,  fill=as.factor(testtot.above.mean) , color=as.factor(testtot.above.mean))) +
  geom_smooth (data=newdat.discrete.cont, method="glm", formula = y~x+I(x^2),  alpha=0.65, size = 1,level=0.95, aes(milestones, predict_probability,  fill=as.factor(testtot.above.mean) , color=as.factor(testtot.above.mean))) +
   geom_pointrange(data=pointestpretest,aes(time,exp(logith)/(1+exp(logith)), ymin=exp(logith)/(1+exp(logith))-0.06, ymax=exp(logith)/(1+exp(logith))+0.06, color=as.factor(abovemedian)))
```
