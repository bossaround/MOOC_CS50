...
```{r}
datm <- dat9
datm$age <- as.numeric(datm$age)
datm$testtot <- as.numeric(datm$testtot)
datm$extrovert <- as.numeric(datm$extrovert)
datm$gamehour5 <- as.numeric(datm$gamehour5)
datm$csexperience <- as.numeric(datm$csexperience)
datm$motiv <- as.numeric(datm$motiv)

```
```{r}
timevaryingmodel2 <- glm(drop ~ milestone + milestone2+ milestone3 + male + age + testtot  +foreign + extrovert+ gamehour5 + csexperience + motiv  + testtot*milestone + male*milestone2  + age*milestone, family="binomial", data=datm)
...
datC <- datK
save(datC, datL, pointestm, file = 'male_cubic_linear_point_data.RData')
```
```{r}
# use smooth to add a nonparametric curve.
...
```
```{r results="asis",warning=FALSE,message=FALSE}
xafit1<-glm(drop~foreign, family="binomial", data=dat5)
summary(xafit1)
t<-data.frame(xafit1$coefficients)
p0<-t[1,1]
p1<-p0+t[2,1]

ts0 <- survfit( Surv(ms+1, 1-censor)~ 1, conf.type="none", subset=(foreign==0), data=dat3)
ts0
ts1 <- survfit( Surv(ms, 1-censor)~ 1, conf.type="none", subset=(foreign==1), data=dat3)
ts1
...
#logith1<-odds1
dat3$ms2 <- dat3$ms^2
xafit8<-glm(event~foreign + ms +ms2, family="binomial", data=dat3)
summary(xafit8)
